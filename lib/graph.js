 ...
 */
module.exports = function writeGraph(fileName, results, units) {
  ...
  return phantom.create(['--web-security=no'], {dnodeOpts: {weak: false}})
    ...
    .then(() => {
      ...
      // Set content of page
      return page.setContent(htmlTemplate({results, colors}), '');
    })
    .then(() => {
      // Inject D3 in sandboxed page
      page.injectJs(path.join(__dirname, '../node_modules/d3/d3.js'));
      return page.evaluate(d3graph, results, points, units, colors);
    })
    .then(() => {
